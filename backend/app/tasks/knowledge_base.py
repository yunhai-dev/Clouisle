...
def process_document_task(self, document_id: str) -> dict:
    ...
    async def _process():
        ...
        # Get document
        document = await Document.filter(id=doc_uuid).prefetch_related(
            "knowledge_base"
        ).first()
        ...
        try:
            ...
            chunk_size = doc_meta.get("chunk_size") or kb_settings.get("chunk_size", 500)
            chunk_overlap = doc_meta.get("chunk_overlap") or kb_settings.get("chunk_overlap", 50)
...
def rechunk_document_task(self, document_id: str) -> dict:
    ...
    async def _rechunk():
        ...
        try:
            ...
            logger.info(f"Deleted {deleted_count} chunks for rechunking document {document_id}")
...
def embed_document_chunks_task(self, document_id: str) -> dict:
    ...
    async def _embed():
        ...
        try:
            ...
            chunks = await DocumentChunk.filter(document_id=doc_uuid).order_by("chunk_index")
            
            if not chunks:
                logger.warning(f"No chunks found for document {document_id}")
                return {"status": "success", "message": "No chunks to embed", "embedded_count": 0}
