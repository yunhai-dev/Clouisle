...
class UsageTracker:
    ...
    async def check_and_record_usage(
        ...
        async with in_transaction() as conn:
            ...
            # 检查并重置每日用量
            if (
                team_model.daily_reset_at is None
                or team_model.daily_reset_at < today_start
            ):
                team_model.daily_tokens_used = 0
                team_model.daily_requests_used = 0
                team_model.daily_reset_at = current_time
            ...
            # 检查日 Token 配额
            if team_model.daily_token_limit is not None and (
                team_model.daily_tokens_used + tokens_used
                > team_model.daily_token_limit
            ):
                raise QuotaExceededError(
            ...
            # 检查月 Token 配额
            if team_model.monthly_token_limit is not None and (
                team_model.monthly_tokens_used + tokens_used
                > team_model.monthly_token_limit
            ):
                raise QuotaExceededError(
            ...
            # 检查日请求配额
            if team_model.daily_request_limit is not None and (
                team_model.daily_requests_used + request_count
                > team_model.daily_request_limit
            ):
                raise QuotaExceededError(
                    f"Daily request quota exceeded. "
                    f"Used: {team_model.daily_requests_used}, "
            ...
            # 检查月请求配额
            if team_model.monthly_request_limit is not None and (
                team_model.monthly_requests_used + request_count
                > team_model.monthly_request_limit
            ):
                raise QuotaExceededError(
                    f"Monthly request quota exceeded. "
                    f"Used: {team_model.monthly_requests_used}, "
            ...
            await team_model.save(using_db=conn)
    ...
    async def check_quota_with_model(
        self,
        team_model: TeamModel,
        tokens_needed: int = 0,
    ) -> None:
        """
        使用已获取的 TeamModel 检查配额（避免重复查询数据库）

        Args:
            team_model: 已获取的团队模型授权记录
            tokens_needed: 预计需要的 token 数量（用于预检查）
        ...
        if not team_model.is_enabled:
            raise ValueError(
                f"Model authorization is disabled for team {team_model.team_id}"
            )
