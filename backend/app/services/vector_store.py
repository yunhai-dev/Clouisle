...
class VectorStore:
    ...
    async def embed_texts(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            embeddings = await model_manager.embed(texts, model_id=self.embedding_model_id)
            return embeddings
    ...
    async def embed_query(self, query: str) -> list[float]:
        ...
        """
        try:
            embedding = await model_manager.embed_query(query, model_id=self.embedding_model_id)
            return embedding
        except Exception as e:
    ...
    async def search(
        ...
        if search_mode == "vector":
            ...
            results = await self._fulltext_search(kb_id, query, top_k * 2, filter_doc_ids)
        else:  # hybrid
            # Get results from both methods
            vector_results = await self._vector_search(kb_id, query, top_k, filter_doc_ids)
            fulltext_results = await self._fulltext_search(kb_id, query, top_k, filter_doc_ids)
    ...
    async def _vector_search(
        ...
        for chunk in chunks:
            ...
            score = self._quick_similarity_score(query_lower, query_terms, chunk.content.lower())
    ...
    def _quick_similarity_score(self, query: str, query_terms: list[str], content: str) -> float:
        """
        ...
        matches = sum(1 for term in query_terms if term in content_lower or term.lower() in content_lower)
        if matches == 0:
            return 0.0
    ...
    def _tokenize(self, text: str) -> set[str]:
        ...
        for word in words:
            ...
            if re.match(r'^[\s\W]+$', word) and not any('\u4e00' <= c <= '\u9fff' for c in word):
                continue
    ...
    async def delete_kb_vectors(self, kb_id: UUID) -> int:
        ...
        documents = await Document.filter(knowledge_base_id=kb_id).values_list("id", flat=True)
