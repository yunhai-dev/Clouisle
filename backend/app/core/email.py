...
async def generate_verification_code(email: str, purpose: str = "register") -> tuple[str, str]:
    """
    生成邮箱验证码和 token
...
async def check_email_cooldown(email: str, purpose: str = "register") -> tuple[bool, int]:
    """
    检查邮件发送冷却时间
...
async def send_verification_email(email: str, code: str, token: str, purpose: str = "register"):
    """
    发送验证码邮件
    """
...
async def check_bulk_email_rate(sender_id: str, max_per_hour: int = 100) -> tuple[bool, int, int]:
    """
    检查管理员批量发送邮件的速率限制
...
async def check_recipient_email_rate(email: str, max_per_day: int = 5) -> tuple[bool, int]:
    """
...
async def filter_rate_limited_recipients(emails: list[str], max_per_day: int = 5) -> tuple[list[str], list[str]]:
    """
    过滤超出速率限制的收件人
